...
export default function ThaIdolApp() {
  ...
  return <main className="max-w-[1100px] mx-auto p-3 space-y-4 bg-[#fffafd]">
    ...
    <section className="bg-white border rounded-xl p-3" style={{borderColor:C.border}}><h2 className="font-semibold mb-2">今週のおすすめライブ</h2><div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3">{weekly.map(e=><div key={e.id} className="space-y-1"><Card title={e.titleJapanese} imageUrl={e.imageUrl} href={`/events/${e.slug}/`} subtitle={`${e.date} ${formatPrice(e)}`}/><div className="flex gap-2 text-xs"><a href={e.ticketUrl} className="underline">チケット</a><a href={e.mapUrl} className="underline">地図</a></div></div>)}</div></section>
    ...
    <section className="grid md:grid-cols-3 gap-3"><div className="bg-white border rounded-xl p-3" style={{borderColor:C.border}}><h2 className="font-semibold mb-2">イベント</h2>{EVENTS.map(e=><Link key={e.id} href={`/events/${e.slug}/`} className="block text-xs underline mb-1">{e.titleJapanese}</Link>)}</div><div className="bg-white border rounded-xl p-3" style={{borderColor:C.border}}><h2 className="font-semibold mb-2">アイドル</h2>{IDOLS.map(i=><Link key={i.id} href={`/idols/${i.slug}/`} className="block text-xs underline mb-1">{i.name}</Link>)}</div><div className="bg-white border rounded-xl p-3" style={{borderColor:C.border}}><h2 className="font-semibold mb-2">運営Lab</h2>{LAB_ARTICLES.map(a=><Link key={a.id} href={`/lab/${a.slug}/`} className="block text-xs underline mb-1">{a.titleJapanese}</Link>)}<Link href="/media/" className="block text-xs underline mt-2">ファン向け記事一覧へ</Link></div></section>
  </main>;
}
